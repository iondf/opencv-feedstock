...
if [[ "${target_platform}" == "linux-64" && -n "${cuda_version:-}" && "${cuda_version}" != "None" ]]; then
    ...
    CMAKE_ARGS="${CMAKE_ARGS} -DCUDA_NVCC_EXECUTABLE=${PREFIX}/bin/nvcc"
    CMAKE_ARGS="${CMAKE_ARGS} -DCMAKE_CUDA_COMPILER=${PREFIX}/bin/nvcc"
    CMAKE_ARGS="${CMAKE_ARGS} -DCMAKE_CUDA_HOST_COMPILER=${CXX}"
