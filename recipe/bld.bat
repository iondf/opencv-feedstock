...
if defined cuda_version (
    if not "%cuda_version%"=="None" (
        set CUDA=1
        set CUBLAS=1
        set CUDA_EXTRA_ARGS=-DCUDA_ARCH_BIN=75;86;89 -DCUDA_ARCH_PTX= -DCUDA_TOOLKIT_ROOT_DIR=%UNIX_LIBRARY_PREFIX% -DOPENCV_DNN_CUDA=OFF -DENABLE_CUDA_FIRST_CLASS_LANGUAGE=ON -DCUDA_USE_STATIC_CUDA_RUNTIME=OFF -DCMAKE_CUDA_RUNTIME_LIBRARY=Shared -DWITH_CUDNN=OFF
    )
)
